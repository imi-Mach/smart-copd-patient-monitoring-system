<template>
  <div class="page page--about">
    <div class="toggle">
      <span @click="toggleTheme">Change the theme</span>
      <div class='user' v-if="getCookie('profile')">
        <router-link to="/user-profile" class="new-review" id="profile-button" v-if="getCookie('profile') != 'Guest'">👋 {{ getCookie('profile') }}</router-link>
        <router-link class="new-review" to="/login">Back to Login Page</router-link>
      </div>
    </div>
    <div class="title">
      <h1>{{ title }}</h1>
      <div id="write-review" class="course-links">
